...
def build_vocab(opts):
    ...
    dataloader.write_word2id_to_file(word2id, opts.out)
...
def init(opts, dataloader):
    encoder_class = encoder_decoder.Encoder
    if opts.with_attention:
        decoder_class = encoder_decoder.DecoderWithAttention
    elif opts.with_multihead_attention:
        decoder_class = encoder_decoder.DecoderWithMultiHeadAttention
    else:
        decoder_class = encoder_decoder.DecoderWithoutAttention
    return encoder_decoder.EncoderDecoder(
...
def train(opts):
    ...
    french_word2id = dataloader.read_word2id_from_file(opts.french_vocab)
    english_word2id = dataloader.read_word2id_from_file(opts.english_vocab)
    ...
    while epoch <= max_epochs and num_poor < patience:
        model.train()
        loss = training_and_testing.train_for_epoch(
            model, train_dataloader, optimizer, opts.device)
        model.eval()
        with torch.no_grad():
            bleu = training_and_testing.compute_average_bleu_over_dataset(
